...
#: Where a reader who was forwarded this file can get their own copy. These
#: reports travel: an integrator runs one, drops the HTML into an email and
#: sends it to a vendor's firmware team, who have never heard of this tool.
#: The footer is the only thing that tells them what they are reading and
...
footer.rep .cta{background:var(--accent-dim);border:1px solid #cadefa;
  border-radius:var(--r-sm);padding:12px 14px;color:var(--muted)}
footer.rep .fine{font-size:12px;color:var(--dim);margin:0}
@media print{
...
<footer class="rep"><div class="wrap">
  ...
  conformance tester, part of <a href="{_e(HOME_URL)}">{_e(PROJECT)}</a> by
  <a href="{_e(HOME_URL)}">{_e(VENDOR)}</a>. It observes and probes a vehicle
  over MQTT and grades what it actually saw — it does not read your source, and
  it never invents a position or a node id it was not told about.</p>
  ...
  rule gets fixed.</p>
  <p class="cta">Was this forwarded to you? Run it yourself against your own
  broker, free and without talking to anyone:
  <a href="{_e(TOOL_URL)}">{_e(TOOL_URL)}</a></p>
  <p class="fine">Generated by {_e(document['tool'])} {_e(document['tool_version'])}
  against {_e(document['spec'])} · report schema {_e(document['schema_version'])} ·
  <a href="{_e(HOME_URL)}">yantrika.ai</a></p>
</div></footer>
<script>{_JS}</script>
</body></html>
"""
